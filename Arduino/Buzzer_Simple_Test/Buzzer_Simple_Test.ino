/*
* Buzzer Arduino Tutorial 
* by @PastorEdu
*/

// 振动频率与音符对应表
//    http://www.phy.mtu.edu/~suits/notefreqs.html
// 下面这一堆 #define 都是把振动的数值对应上音符的，因为音符我们能看懂，但数值却很难记住
...
// 这里把蜂鸣器的输出接到了 D13，可以根据自己的需求设置，接A0 - A3也可以
...
void _tone (float noteFrequency, long noteDuration, int silentDuration){
    ...
    // 这个函数控制单个音符，就好像钢琴里的一个琴键一样

      if(silentDuration==0){silentDuration=1;}

      tone(PIN_Buzzer, noteFrequency, noteDuration); //奏起音符
      delay(noteDuration);                           //持续一段时间
      noTone(PIN_Buzzer);                            //停止音符
      delay(silentDuration);                         //持续一段时间
}
